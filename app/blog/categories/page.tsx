...
export const metadata: Metadata = {
  ...
  description:
    "Browse all blog categories and discover content that interests you.",
};
...
export default async function CategoriesPage() {
  try {
    ...
    // Add fallback if categories or posts are undefined
            ...
            {safeCategories.length === 0 ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {safeCategories.map((category) => {
                  // Safely count posts for this category
                  const postCount = safePosts.filter(
                    (post) => post && post.category_id === category.id
                  ).length;
                    ...
                    >
                      <div className="relative h-40 w-full overflow-hidden">
                        ...
                        <Image
                          src={
                            category.image_url ||
                            `/placeholder.svg?height=400&width=600&query=${category.name} category`
                          }
                        ...
                        <div className="absolute bottom-4 left-4 right-4 z-20">
                          <div
                            className={`h-1.5 w-12 rounded-full bg-gradient-to-r ${
                              category.color || "from-blue-500 to-cyan-500"
                            } mb-3`}
                          ></div>
      ...
      <main className="min-h-screen bg-[#0a1218] text-white">
        ...
        <section className="py-16 md:py-24">
          <div className="container px-4 md:px-6">
            ...
            <h1 className="text-4xl font-bold mb-4">Blog Categories</h1>
            <div className="bg-red-900/30 border border-red-900 rounded-lg p-6 mt-8">
              <h2 className="text-lg font-medium mb-2">
                Unable to load categories
              </h2>
              <p className="text-muted-foreground">
                There was a problem loading the blog categories. Please try
                again later.
              </p>
            </div>
          </div>
