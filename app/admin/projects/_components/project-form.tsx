...
import type { Project, ProjectChallenge, ProjectMilestone, Tag } from "@/types/projects"
...
export function ProjectForm({ project, tags, selectedTagIds, challenges, milestones, action }: ProjectFormProps) {
  ...
  const handleAddMilestone = () => {
    setNewMilestones([...newMilestones, { description: "", due_date: null, completed: false }])
  }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    newMilestones.forEach((milestone, index) => {
      if (milestone.description.trim()) {
        ...
        formData.append("new_milestone_due_date", milestone.due_date ? format(milestone.due_date, "yyyy-MM-dd") : "")
        formData.append("new_milestone_completed", milestone.completed ? "true" : "false")
      }
    })
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="team_size" name="team_size" type="number" min="1" defaultValue={project?.team_size || 1} />
                </div>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0">
                      <Calendar
                        mode="single"
                        initialFocus
                        defaultMonth={project?.start_date ? new Date(project.start_date) : undefined}
                  ...
                  {project?.start_date && (
                    <input
                      type="hidden"
                      name="start_date"
                      defaultValue={format(new Date(project.start_date), "yyyy-MM-dd")}
                    />
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {project?.end_date ? format(new Date(project.end_date), "PPP") : "Select date"}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0">
                      <Calendar
                        mode="single"
                        initialFocus
                        defaultMonth={project?.end_date ? new Date(project.end_date) : undefined}
                  ...
                  {project?.end_date && (
                    <input
                      type="hidden"
                      name="end_date"
                      defaultValue={format(new Date(project.end_date), "yyyy-MM-dd")}
                    />
                  )}
                </div>
              </div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="demo_url" name="demo_url" defaultValue={project?.demo_url || ""} />
                </div>
              </div>
        ...
        <TabsContent value="content" className="space-y-4 mt-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="detailed_description">Detailed Description (Markdown)</Label>
                <Textarea
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded-md p-4">
                  <ImageUploader currentImage={imageUrl} onImageSelected={handleImageSelected} folder="projects" />
            ...
            <CardContent className="space-y-4">
              {challenges.map((challenge, index) => (
                <div key={challenge.id} className="space-y-2">
                  <Label htmlFor={`challenge-${challenge.id}`}>Challenge {index + 1}</Label>
              ...
              {newChallenges.map((challenge, index) => (
                <div key={`new-${index}`} className="space-y-2">
                  <Label htmlFor={`new-challenge-${index}`}>New Challenge {index + 1}</Label>
                  <div className="flex gap-2">
                    <Textarea
              ...
              <Button type="button" onClick={handleAddChallenge} className="mt-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {milestones.map((milestone, index) => (
                <div key={milestone.id} className="space-y-2 border-b pb-4 mb-4 last:border-0">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="md:col-span-2">
                      <Label htmlFor={`milestone-${milestone.id}`}>Milestone {index + 1}</Label>
                      <Textarea
                    ...
                    <div className="space-y-2">
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          ...
                          >
                            <CalendarIcon className="mr-2 h-4 w-4" />
                            {milestone.due_date ? format(new Date(milestone.due_date), "PPP") : "Select date"}
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-auto p-0">
                          <Calendar
                            mode="single"
                            initialFocus
                            defaultMonth={milestone.due_date ? new Date(milestone.due_date) : undefined}
                      ...
                      {milestone.due_date && (
                        <input
                          ...
                          defaultValue={format(new Date(milestone.due_date), "yyyy-MM-dd")}
                        />
                      )}
                      ...
                      <div className="flex items-center space-x-2 mt-2">
                        ...
                        />
                        <Label htmlFor={`milestone-completed-${milestone.id}`} className="font-normal">
                          Completed
                        </Label>
                      </div>
                    </div>
              ...
              {newMilestones.map((milestone, index) => (
                <div key={`new-milestone-${index}`} className="space-y-2 border-b pb-4 mb-4 last:border-0">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="md:col-span-2">
                      <Label htmlFor={`new-milestone-${index}`}>New Milestone {index + 1}</Label>
                      <div className="flex gap-2">
                        <Textarea
                    ...
                    <div className="space-y-2">
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          ...
                          >
                            <CalendarIcon className="mr-2 h-4 w-4" />
                            {milestone.due_date ? format(milestone.due_date, "PPP") : "Select date"}
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-auto p-0">
                          <Calendar
                            ...
                            onSelect={(date) => handleMilestoneChange(index, "due_date", date)}
                          />
                        </PopoverContent>
                      </Popover>
                      <input
                        ...
                        value={milestone.due_date ? format(milestone.due_date, "yyyy-MM-dd") : ""}
                      />
                      ...
                      <div className="flex items-center space-x-2 mt-2">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => handleMilestoneChange(index, "completed", checked === true)}
                        />
                        <Label htmlFor={`new-milestone-completed-${index}`} className="font-normal">
                          Completed
                        </Label>
                      </div>
                    </div>
              ...
              <Button type="button" onClick={handleAddMilestone} className="mt-2">
                <Plus className="h-4 w-4 mr-2" />
                Add Milestone
              </Button>
            </CardContent>
