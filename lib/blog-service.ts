...
import type { BlogPost, BlogPostWithCategory, BlogPostWithCategoryAndTags, Category } from "@/types/blog"
...
export async function getPublishedPosts(
  ...
  try {
    return await retryOperation(async () => {
      ...
      if (error) {
        throw handleDatabaseError(error, "fetch", "published posts", { page, limit, featured })
      }
...
export async function getFeaturedPosts(limit = 3): Promise<BlogPostWithCategory[]> {
  try {
...
export async function getPostBySlug(slug: string): Promise<BlogPostWithCategoryAndTags | null> {
  if (!slug) {
  ...
  try {
    return await retryOperation(async () => {
      ...
      if (error) {
        ...
        throw handleDatabaseError(error, "fetch", `blog post with slug ${slug}`, { slug })
      }
      ...
      const transformedTags = post.blog_post_tags.map((tagJoin: any) => tagJoin.blog_tags)
...
export async function getPostsByCategory(
  ...
  page = 1,
  limit = 10,
): Promise<{ posts: BlogPostWithCategory[]; total: number; category: Category | null }> {
  if (!slug) {
  ...
  try {
    return await retryOperation(async () => {
      ...
      if (categoryError) {
        ...
        throw handleDatabaseError(categoryError, "fetch", `category with slug ${slug}`, { slug })
      }
      ...
      if (postsError) {
        throw handleDatabaseError(postsError, "fetch", `posts for category ${slug}`, {
...
export async function createPost(post: Partial<BlogPost>): Promise<BlogPost | null> {
  try {
    ...
    validateRequiredFields(post, ["title", "slug", "content", "category_id"], "blog post")
...
export async function updatePost(id: number, post: Partial<BlogPost>): Promise<BlogPost | null> {
  ...
  try {
    return await retryOperation(async () => {
      ...
      if (error) {
        throw handleDatabaseError(error, "update", `blog post with ID ${id}`, { id, post })
      }
...
export async function deletePost(id: number): Promise<boolean> {
  ...
  try {
    return await retryOperation(async () => {
      ...
      if (error) {
        throw handleDatabaseError(error, "delete", `blog post with ID ${id}`, { id })
      }
...
export async function getPostsByTag(
  ...
  try {
    return await retryOperation(async () => {
      ...
      // First, get the tag
      const { data: tag, error: tagError } = await supabase.from("blog_tags").select("*").eq("slug", slug).single()
      ...
      if (tagError) {
        ...
        throw handleDatabaseError(tagError, "fetch", `tag with slug ${slug}`, { slug })
      }
      ...
      if (postIdsError) {
        throw handleDatabaseError(postIdsError, "fetch", `post IDs for tag ${slug}`, { tagId: tag.id })
      }
      ...
      if (postsError) {
        throw handleDatabaseError(postsError, "fetch", `posts for tag ${slug}`, {
          tagId: tag.id,
          page,
          limit,
        })
