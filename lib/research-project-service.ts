  ...
  try {
    return await retryOperation(async () => {
      ...
      return data.map((project) => {
        // Keep both snake_case and camelCase versions for compatibility
        return {
          ...project,
...
export async function getFeaturedResearchProjects(limit = 3): Promise<ResearchProject[]> {
  try {
    return await retryOperation(async () => {
      const supabase = createServerSupabaseClient()
...
export async function getResearchProjectById(id: number): Promise<ResearchProject | null> {
  try {
    return await retryOperation(async () => {
      const supabase = createServerSupabaseClient()
      ...
      if (error) {
        throw handleDatabaseError(error, "fetch", `research project with ID ${id}`)
      }
...
export async function createResearchProject(project: Partial<ResearchProject>): Promise<ResearchProject | null> {
  try {
    return await retryOperation(async () => {
      const supabase = createServerSupabaseClient()
...
export async function updateResearchProject(
  ...
  try {
    return await retryOperation(async () => {
      ...
      if (error) {
        throw handleDatabaseError(error, "update", `research project with ID ${id}`)
      }

      // Handle tags (delete and re-insert)
      if (project.tags) {
        // Delete existing tags
        await supabase.from("research_project_tags").delete().eq("research_project_id", id)
      ...
      if (project.challenges) {
        ...
        await supabase.from("research_project_challenges").delete().eq("research_project_id", id)
      ...
      if (project.recentUpdates) {
        ...
        await supabase.from("research_project_updates").delete().eq("research_project_id", id)
      ...
      if (project.teamMembers) {
        ...
        await supabase.from("research_project_team_members").delete().eq("research_project_id", id)
      ...
      if (project.resources) {
        ...
        await supabase.from("research_project_resources").delete().eq("research_project_id", id)
...
export async function deleteResearchProject(id: number): Promise<boolean> {
  try {
    return await retryOperation(async () => {
      ...
      if (error) {
        throw handleDatabaseError(error, "delete", `research project with ID ${id}`)
      }
...
export async function moveToProjects(id: number): Promise<boolean> {
  try {
    return await retryOperation(async () => {
        ...
        .insert([
          {
            ...
            end_date: researchProject.endDate || new Date().toISOString().split("T")[0],
            priority: researchProject.priority,
            image_url: researchProject.image_url,
            technologies: researchProject.tags,
            key_achievements: researchProject.recentUpdates.map((update) => update.text),
            role:
              researchProject.teamMembers.find((member) => member.includes("(Lead)"))?.replace(" (Lead)", "") ||
              "Researcher",
          },
        ])
        .select()
      ...
      if (error) {
        throw handleDatabaseError(error, "create", "project from research project")
      }
      ...
      await supabase.from("research_projects").update({ is_active: false }).eq("id", id)
...
export async function getResearchProjectBySlug(slug: string): Promise<ResearchProject | null> {
  try {
    return await retryOperation(async () => {
      ...
      if (error) {
        throw handleDatabaseError(error, "fetch", `research project with slug ${slug}`)
      }
