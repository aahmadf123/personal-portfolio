...
export async function updateResearchProject(id: number, formData: FormData) {
  ...
  const daysRemaining =
    Number.parseInt(formData.get("days_remaining") as string) || 0;
  ...
  if (
    ...
    resourceNames.length === resourceUrls.length
  ) {
