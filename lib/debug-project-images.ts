...
export async function debugProjectImages(
  ...
  try {
    ...
    // Compute image_url from other fields
    const imageUrl = project.thumbnail_url || project.main_image_url || null;
    console.log(`- computed image_url: ${imageUrl || "none"}`);

    if (project.project_images && project.project_images.length > 0) {
      console.log(`Project images (${project.project_images.length}):`);
      project.project_images.forEach((img, i) => {
        console.log(
          `  ${i + 1}. ${img.url || "no URL"} (is_main: ${
            img.is_main || false
          })`
        );
      });
...
export async function getProjectWithImages() {
  try {
    ...
    projects.forEach((p) => {
      // Compute image_url for each project
      const imageUrl = p.thumbnail_url || p.main_image_url || null;
