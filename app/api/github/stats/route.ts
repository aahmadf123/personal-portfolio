...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!username) {
      throw new Error(
        "Could not determine GitHub username from environment variables"
      );
    }
    ...
    if (!githubToken) {
      throw new Error(
        "GITHUB_TOKEN is required for fetching GitHub statistics"
      );
    }
    ...
    const languageStats = Object.entries(languages).map(
      ([name, { count, color }]) => ({
        name,
        count,
    ...
    const mostActiveDay = [...commitFrequency].sort(
      (a, b) => b.count - a.count
    )[0].day;

    // Calculate repository growth over time
    const reposByDate = repositories.reduce(
    ...
    const initialTotalStars = repositories.reduce(
      (sum: number, repo: any) => sum + repo.stars,
      0
    );
    ...
    for (let i = 0; i < totalDays; i += Math.floor(totalDays / 20)) {
      ...
      currentStars += Math.floor(
        starsPerDay * i * (Math.random() * 0.5 + 0.75)
      ); // Add some randomness
    ...
    const topRepositories = repositories
      ...
      .map((repo: any) => ({
        ...
        languageColor: repo.language
          ? languageColors[repo.language] || "#8884d8"
          : "#8884d8",
      }));
    ...
    const totalForks = repositories.reduce(
      (sum: number, repo: any) => sum + repo.forks,
      0
    );
    const totalWatchers = repositories.reduce(
      (sum: number, repo: any) => sum + (repo.watchers_count || 0),
      0
    );
